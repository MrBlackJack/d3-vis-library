var viz = function ($element, layout, _this) {
    ...
    color.domain(data.map(function (d) {
        return d.dim(2).qText
    }));

    var nested_data = d3.nest()
        .key(function (d) {
            return d.dim(1).qText
        })
        .entries(data);
    ...
    var legend_width = d3.max(svg.selectAll(".legend_temp")[0], function (d) {
    ...
    dim1.selectAll("rect")
        ...
        //        .attr("x", function (d) {
        //            return x(d.measure(1).y1);
        //        })
        //        .attr("height", y.rangeBand())
        //        .attr("width", function (d) {
        //            return x(d.measure(1).y0) - x(d.measure(1).y1);
        //        })
        //Вертикальная ориентация
        .attr("y", function (d) {
            return y(d.measure(1).y1);
        })
        ...
        .attr("height", function (d) {
            return y(d.measure(1).y0) - y(d.measure(1).y1);
        })
        //Заливка
        .style("fill", function (d) {
            return color(d.dim(2).qText);
        })
        .on("click", function (d) {
            d.dim(2).qSelect();
        });
